...
const imageSources = [
    ...
    {
        suffix: '_md',
        media: '(max-width: 640px),(min-width: 1101px) and (max-width: 1280px)'
    }
];
...
const fillRestaurantHTML = (restaurant = self.restaurant) => {
    ...
    imageSources.forEach((imageSource) => {
        ...
        source.srcset = DBHelper.imageUrlForRestaurant(restaurant.photograph, '/responsive/', imageSource.suffix);
        source.media = imageSource.media;
        sources.push(source);
    });
...
const fillRestaurantHoursHTML = (operatingHours = self.restaurant.operating_hours) => {
    ...
    for (let key in operatingHours) {
        ...
        day.innerHTML = key;
        day.setAttribute("aria-label", "Open " + key);
        row.appendChild(day);
...
const addReview =  () => {
    ...
    const rating = document.querySelector('#rating_select option:checked').value;
    const comments = document.getElementById('comments').value;

    if (!validateField(name, 'name') || !validateField(comments, 'comments')) {
        return;
    }
    ...
    DBHelper.addReview(userInput);
    const ul = document.getElementById('reviews-list');
    ul.appendChild(createReviewHTML(userInput));
    document.getElementById('form').reset();
};

document.getElementById('form_submit').addEventListener('click', addReview);
